...
export default function DigitalTwinRing({ today }) {
  ...
  const hrv = Math.round(today.hrv ?? 0);
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative" style={{ width: size, height: size }}>
        ...
        />
        <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }} role="img" aria-label={`Gemelo digital: índice de riesgo ${score} de 100, nivel ${level}`}>
          <defs>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span style={{ color: C.textFaint }} className="text-[10px] uppercase tracking-[0.2em] mb-1">Gemelo digital</span>
          ...
          <span style={{ color: C.textFaint }} className="text-[11px] -mt-0.5">/ 100</span>
          <span style={{ color, fontFamily: "'IBM Plex Mono', monospace" }} className="text-[11px] font-semibold mt-1.5 tracking-wide">{level}</span>
          <span style={{ color: C.textMuted }} className="text-[10px] mt-1">HRV {hrv} ms</span>
        </div>
      </div>
